%comparision of eVU and VU with SpaRSA/m and gpsrBB!
clc;clear;
%parameters
pr.sigma=0.0001;pr.epsilon =1e-7;
...
l=length(nameFolds);
fid=fopen('./O_CompOthersV3.txt','a');
fprintf(fid,'Save results of eVUl1 and VUl1 version 3.0 to compare with other 3 solvers ');
...
save('r_compOtherV3.mat','eVU_r','VU_r');
...
fv=[eVU_r(:,1), VU_r(:,1), resSpaRSA(:,1),resSpaRSA_m(:,1),resgpsrBB(:,1)];
ha=perfprof(fv,1.05);
legend(ha,{'\(\varepsilon\)-\(\mathcal{VU}\)','\(\mathcal{VU}\)','SpaRSA','SpaRSAm','gpsrBB'},'Location','southeast','Interpreter','latex');
%legend(ha,{'\(\varepsilon\)-\(\mathcal{VU}\)','\(\mathcal{VU}\)','SpaRSA','SpaRSAm','l1ls','gpsrBB'},'Location','southeast','Interpreter','latex');
figure;
cpu=[eVU_r(:,2), VU_r(:,2),resSpaRSA(:,2),resSpaRSA_m(:,2),resgpsrBB(:,2)];
%cpu=[eVU_r(:,2), VU_r(:,2),resSpaRSA(:,2),resSpaRSA_m(:,2),resl1_ls(:,2),resgpsrBB(:,2)];
hb=perfprof(cpu,5);
legend(hb,{'\(\varepsilon\)-VU','\(\mathcal{VU}\)','SpaRSA','SpaRSAm','gpsrBB'},'Location','southeast','Interpreter','latex');
