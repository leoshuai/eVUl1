...
l=length(nameFolds);
fid=fopen('./OutputCompadmm1.txt','a');
...
save('results_compadmm.mat','eVU_r','VU_r');
...
% load('otherResults.mat');
 fv=[eVU_r(:,1), VU_r(:,1), admm(:,1)];
ha=perfprof(fv,1.05);
% fv=[eVU_r(:,1), VU_r(:,1), resSpaRSA(:,1),resSpaRSA_m(:,1),resgpsrBB(:,1),admm(:,1)];
% ha=perfprof(fv,1.05
 legend(ha,{'\(\varepsilon\)-\(\mathcal{VU}\)','\(\mathcal{VU}\)','admm'},'Location','southeast','Interpreter','latex');
...
% cpu=[eVU_r(:,2), VU_r(:,2),resSpaRSA(:,2),resSpaRSA_m(:,2),resl1_ls(:,2),resgpsrBB(:,2)];
 hb=perfprof(cpu,5);
 legend(hb,{'\(\varepsilon\)-VU','\(\mathcal{VU}\)','admm'},'Location','southeast','Interpreter','latex');
